...
class PhysicsChecks:
    ...
    def check_r_values(self):
        ...
        denom = ref_stress
        dg11 = 2*(G*s11_b + H*(s11_b-s22_b)) / denom
        dg22 = 2*(F*s22_b - H*(s11_b-s22_b)) / denom
        dg12 = 2*(2*N*s12_b) / denom
        
        # R-value formula using gradients
        d_eps_w = dg11*sin_a**2 + dg22*cos_a**2 - 2.0*dg12*sin_a*cos_a
        d_eps_t = -(dg11 + dg22)
        ...
        # Calculate R-value
        r_nn = (ds_11*sin_a**2 + ds_22*cos_a**2 - 2.0*ds_12*sin_a*cos_a) / (-(ds_11 + ds_22) + 1e-8)
