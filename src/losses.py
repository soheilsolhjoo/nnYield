...
class PhysicsLoss:
    ...
    def calculate_losses(self, model, inputs, targets, weights, run_convexity=False, run_symmetry=False):
        ...
        l_conv_static = tf.constant(0.0, dtype=tf.float32)
        l_conv_dynamic = tf.constant(0.0, dtype=tf.float32)
        l_sym = tf.constant(0.0, dtype=tf.float32)
        min_eig_val = tf.constant(0.0, dtype=tf.float32)

        # Initialize pred_s as a tensor to prevent "None" branch errors
        pred_s = tf.zeros((0, 1), dtype=tf.float32)
        if has_shape_data:
        ...
        if run_convexity and has_shape_data:
            ...
            if weights.convexity > 0:
                l_conv_static = raw_penalty
            if weights.dynamic_convexity > 0:
                l_conv_dynamic = raw_penalty
